 ...
 --   @PsCla_Periodo             Varchar(Max)   = '3',
 --   @PsNominas                 Varchar(Max)   = '202530001',
 ...
 --Begin
 --   Execute dbo.SP_NOMINA_TABULAR_RHPAY @PsRazon_Social          = @PsRazon_Social,
...
Create Or Alter  Procedure SP_NOMINA_TABULAR_RHPAY
  (@PsRazon_Social            Varchar(Max)   = '',
...
Declare
   ...
   @w_dias_por_pagar          Decimal(18, 2) = 0.00,
   @w_dias_por_pagar_ante     Decimal(18, 2) = 0.00,
   @w_columna                 Sysname,
...
--
  ...
  (CLA_EMPRESA       Integer        Not Null,
   ...
   NUM_NOMINA        Integer        Not Null,
   ANIO_MES          Integer        Not Null,
   DIAS_POR_PAGAR    Decimal(18, 2) Not Null Default 0.00,
   ...
   If Isnull(@PsCla_PerDed, '') = ''
      Begin
        ...
        (CLA_EMPRESA,   CLA_PERDED, NOM_PERDED,      TIPO_PERDED,
         ...
         Union
         Select Distinct a.CLA_EMPRESA, a.CLA_PERDED, Substring(a.NOM_PERDED, 1, 10),
                1, Isnull(a.MOSTRAR_SALDO, 0), Isnull(a.GRUPO_CAL1, 0),
         ...
         And    a.NO_IMPRIMIR     = 0
         And    a.ES_PROVISION    = 0
         And CLA_PERDED          In (1048, 1051, 1052)
         Union
         Select Distinct a.CLA_EMPRESA, a.CLA_PERDED, Substring(a.NOM_PERDED, 1, 40),
   ...
   Else
      Begin
        ...
        (CLA_EMPRESA,   CLA_PERDED, NOM_PERDED, TIPO_PERDED,
         ...
         Union
         Select Distinct b.CLA_EMPRESA, b.CLA_PERDED, Substring(b.NOM_PERDED, 1, 40),                      3, Isnull(b.MOSTRAR_SALDO, 0),
                Isnull(b.GRUPO_CAL1, 0), Isnull(b.NO_AFECTAR_NETO, 0),
                Isnull(b.ESBASE_ISPT, 0)
         From   String_split(@PsCla_PerDed, ',') a
         Join   dbo.RH_PERDED                    b
      ...
      Begin
        ...
        (CLA_RAZON_SOCIAL, CLA_EMPRESA,      CLA_TRAB, NOM_TRAB,
         ...
         Select Distinct a.CLA_RAZON_SOCIAL,  a.CLA_EMPRESA, a.CLA_TRAB,
                Concat(Trim(a.AP_PATERNO), ' ', Trim(a.AP_MATERNO) , ' ',   Trim(a.NOM_TRAB)),
...
--
  ...
  (CLA_EMPRESA, CLA_TRAB, CLA_PERIODO, NUM_NOMINA,
   ...
   --       Sum((Isnull(a.DIAS_TRAB, 0)+(Isnull(a.DIAS_VAC, 0) - (Isnull(a.F_JUST_SUE, 0) + Isnull(a.F_INJUST , 0) + Isnull(a.F_JUST_NSUE,0) +
   --                                     Isnull(a.INCAP_ENF , 0) + Isnull(a.INCAP_MAT, 0) + Isnull(a.INCAP_RIES ,0)))) DIAS_POR_PAGAR
          Sum(     Isnull(a.dias_trab, 0) +     Isnull(a.dias_vac, 0) -     (         Isnull(a.F_JUST_SUE, 0) +         Isnull(a.F_INJUST, 0) +         Isnull(a.F_JUST_NSUE, 0) +         Isnull(a.INCAP_ENF, 0) +         Isnull(a.INCAP_MAT, 0) +         Isnull(
                    ...
                    a.INCAP_RIES, 0)     ) ) DIAS_POR_PAGAR -- JM 2025.29.05 - Se ajusta para recuperar los días laborados correctamente--
...
--  And   T.CLA_RAZON_SOCIAL = a.CLA_RAZON_SOCIAL
   ...
   If @PbIncluyeNominaAbierta = 1 Or
      ...
      Begin
        ...
        (CLA_EMPRESA, CLA_TRAB, CLA_PERIODO, NUM_NOMINA,
         ANIO_MES,    DIAS_POR_PAGAR)
         Select a.cla_empresa, a.CLA_TRAB, a.CLA_PERIODO, a.NUM_NOMINA, a.ANIO_MES_ISPT,
                Sum(Isnull(a.dias_lab_calc,   0) +
                    Isnull(a.dias_vac,    0) -
                   (Isnull(a.F_JUST_SUE,  0) +
                    Isnull(a.F_INJUST,    0) +
                    Isnull(a.F_JUST_NSUE, 0) +
                    Isnull(a.INCAP_ENF,   0) +
                    Isnull(a.INCAP_MAT,   0) +
                    Isnull(a.INCAP_RIES,  0) ))
...
--
   ...
   Begin
      ...
      Begin
         ...
         If Isnull(@w_tipo_perdedAnte, 0) = 0
            Begin
               Select @w_suma_Percepciones = 'Update #tmpResultado Set totalPercepciones = 0 ',
...
--
   ...
   Select  @w_regEmpr = Count (distinct CLA_EMPRESA),
           @w_regRZ   = Count (distinct CLA_RAZON_SOCIAL)
   From    #tmpResultado;
...
--
   ...
   Select  @w_regDpto  = Count (distinct CLA_DEPTO)
   From    #tmpResultado;
...
--
   ...
   Select  @w_regCco = Count (distinct CLA_CENTRO_COSTO)
   From    #tmpResultado;
      ...
      Begin
         Set @w_executeInsert = Concat('Insert Into #tmpResultado ',
                                       ...
                                       'Select ', @w_comilla, 'L5', @w_comilla, ', ',
                                               'Max(CLA_RAZON_SOCIAL)  + 1, Max(CLA_EMPRESA) + 1, Max(CLA_TRAB) + 1, ',
                                               'Max(CLA_UBICACION) + 1, ',
                                               'Concat(', @w_comilla, 'Total Razon Social.: ', @w_comilla, ', Max(CLA_RAZON_SOCIAL)), ',
                                               'Max(CLA_CENTRO_COSTO) + 1 , ', 'Max(CLA_DEPTO) +1 ',
                                                @w_colsuma, ' ',
                                       'From  #tmpNominasTrab ',
